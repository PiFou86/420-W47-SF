...
        [HttpGet]
        // PFL : le type est normalement déduit par le compilateur
        // Mais vous pouvez le spécifier explicitement suivant le code de retour
        [ProducesResponseType(200, Type = typeof(IEnumerable<Exemplaire>))]
...
            }
            ...
            // Ce code est fait pour faire une démo sans base de données
            int idMax = Donnees.Values.SelectMany(ce => ce.Select(e => e.ExemplaireId)).OrderByDescending(id => id).FirstOrDefault();
